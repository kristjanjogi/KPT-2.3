...
This schema defines the normalized decision object returned by KPT middleware after evaluating a candidate output.
...
- `rationale` should be present for all production-grade decisions, and is explicitly required by this schema for `qualify`, `refuse_assert`, and `refuse_help`

## Design intent

The schema preserves several KPT 2.3 distinctions:

It is also intentionally narrower than the richer demo/report layer used by the reference engine.
